...
export const printStatement = ({ customer, entries, username, rangeTitle }) => {
  ...
  const html = `
    ...
    <div class="info-grid">
      ...
      <div class="info-row"><span class="lbl">نوع الحساب</span><span class="val">${(customer.customer_type || "customer") === "pos" ? "نقطة بيع" : "عميل"}</span></div>
      <div class="info-row"><span class="lbl">رقم الهاتف</span><span class="val">${customer.phone || "-"}</span></div>
      <div class="info-row"><span class="lbl">الرصيد الحالي</span><span class="val">${(customer.balance||0) === 0 ? "0 ريال" : `${fmt(Math.abs(customer.balance||0))} ${(customer.balance||0) > 0 ? "عليكم" : "لكم"}`}</span></div>
    </div>
    <table>
      <thead><tr>
    ...
    <div class="totals">
      ...
      <div class="row grand"><span>الرصيد النهائي</span><span class="num">${(customer.balance||0) === 0 ? "0 ريال" : `${fmt(Math.abs(customer.balance||0))} ${(customer.balance||0) > 0 ? "عليكم" : "لكم"}`}</span></div>
    </div>
    ${buildFooter(username)}
  `;
  openPrintWindow(html);
