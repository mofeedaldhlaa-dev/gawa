  ...
  return (
    <Card className="overflow-x-auto" data-testid="audit-page">
      <table className="w-full text-sm">
        ...
        <tbody>
          {items.map((a) => (
            <tr key={a.id} className="border-t"><td className="p-3">{fmtDate(a.created_at)}</td><td className="p-3">{a.username}</td><td className="p-3">{a.action}</td><td className="p-3">{a.entity}</td><td className="p-3 text-xs font-mono">{JSON.stringify(a.new_value || a.old_value || {})}</td></tr>
          ))}
        </tbody>
      </table>
    </Card>
