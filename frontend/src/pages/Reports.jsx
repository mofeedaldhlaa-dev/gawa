    ...
    <div className="space-y-4" data-testid="reports-page">
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="flex-wrap h-auto">
          ...
          <TabsTrigger value="supplier_debts" data-testid="rep-sdebts">مديونية نقاط البيع</TabsTrigger>
        ...
        <Card className="p-3 flex gap-2 items-end flex-wrap mt-3 no-print">
          ...
          <Button onClick={() => window.print()} variant="outline" data-testid="print-report"><Printer size={14} className="ml-1"/> طباعة / PDF</Button>
        </Card>

        <div className="print-header hidden print-only text-center mb-3">
          <div className="text-xl font-bold">شبكة جواد نت اللاسلكية</div>
          <div className="text-sm">784225716</div>
          <div className="text-lg font-bold mt-2">{
            {sales:"تقرير المبيعات", purchases:"تقرير المشتريات", electronic:"تقرير المبيعات الإلكترونية",
             card_order_log:"سجل طلبات الرابط", customer_debts:"مديونية العملاء",
             supplier_debts:"مديونية نقاط البيع", stock:"تقرير المخزون"}[tab]
          }</div>
        ...
        <TabsContent value="supplier_debts"><DebtTable data={filtered} label="نقطة البيع"/></TabsContent>
    ...
    <Card className="mt-3 overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {data.length === 0 && <tr><td colSpan={6} className="p-4 text-center text-slate-400">لا توجد بيانات</td></tr>}
        </tbody>
    ...
    <Card className="mt-3 overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {data.length === 0 && <tr><td colSpan={7} className="p-4 text-center text-slate-400">لا توجد بيانات</td></tr>}
        </tbody>
      ...
      <tbody>
        {data.map((a) => (
          <tr key={a.id} className="border-t">
            <td className="p-2">{fmtDate(a.created_at)}</td>
            <td className="p-2">{a.customer_name || "-"}</td>
            <td className="p-2">{a.phone}</td>
            <td className="p-2">{a.category_name || "-"}</td>
            <td className="p-2 num">{a.quantity || 0}</td>
            <td className="p-2 num">{fmt(a.total || 0)}</td>
            <td className="p-2">
              <span className={`text-xs px-2 py-0.5 rounded-full ${a.status==='success'?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}`}>
                {a.status==='success'?'ناجح':a.status==='rejected_over_limit'?'مرفوض - تجاوز السقف':a.status==='rejected_no_stock'?'مرفوض - عدم توفر':'مرفوض - غير موجود'}
              </span>
            </td>
            <td className="p-2 text-xs">{a.reason}</td>
          </tr>
        ))}
        {data.length === 0 && <tr><td colSpan={8} className="p-4 text-center text-slate-400">لا توجد بيانات</td></tr>}
      </tbody>
    </table>
  </Card>
);
    ...
    <Card className="mt-3 overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>{data.map((c) => <tr key={c.id} className="border-t"><td className="p-2">{c.name}</td><td className="p-2">{c.phone}</td><td className="p-2 num">{fmt(c.credit_limit)}</td><td className="p-2 num font-bold">{fmt(c.balance)}</td><td className="p-2 num text-green-600">{fmt(Math.max(0, (c.credit_limit||0) - (c.balance||0)))}</td></tr>)}
          {data.length === 0 && <tr><td colSpan={5} className="p-4 text-center text-slate-400">لا توجد مديونية</td></tr>}
        </tbody>
...
const StockTable = ({ data }) => {
  if (!data || data.length === 0) return <Card className="mt-3 p-6 text-center text-slate-400">لا توجد بيانات مخزون</Card>;
  return (
    <Card className="mt-3 overflow-x-auto">
      <table className="w-full text-sm">
